...
def testv1(method = 'rf'):
    ...
    ifd = pd.read_csv('data_infected.csv')
    hd = hd.dropna()
    ifd = ifd.dropna()
    ...
    wdata = np.concatenate((hdf,ifdf))
    est_labels = np.zeros((len(wdata),))
    wlabel = np.concatenate((hdl,ifdl))
    stratified_kfolds = cv.split(wdata, wlabel)
    importances = []
    
    for trind, teind in stratified_kfolds:
        tr = wdata[trind] 
        trl = wlabel[trind]
        te = wdata[teind]
        tel = wlabel[teind]
        ...
        if method == 'gbt':
           ...
           importances.append(np.reshape(model.coef_,(len(ft,))))
        elif method == 'lr-l2':
    ...
    for f in range(len(indices)):
        sorted_features.append(ft[indices[f]])
    ...
    hdf = hd[:,1:]
    ifdf = ifd[:,1:]
    data = np.concatenate((hdf,ifdf))
    
    # Read features and labels
    hdl = np.zeros((len(hd),))
    ifdl = np.ones((len(ifd),))
    labels = np.concatenate((hdl, ifdl))
...
# Calculate performance (Accuracy, sensitivity, specificity, AUC)
